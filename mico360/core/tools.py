...
TOOLS: list[Tool] = [
    ...
    Tool(
        ...
        options=[
            ...
            # in; everything else is preserved. They hide when "Remove all" is on.
            Option("title", "Title", "text", "", hint="Leave blank to keep existing.",
                   visible_when=("remove_all", False)),
            Option("author", "Author", "text", "", visible_when=("remove_all", False)),
            Option("subject", "Subject", "text", "", visible_when=("remove_all", False)),
            ...
            Option("mod_date", "Modification date", "text", "",
                   hint="e.g. 2026-06-07, or 'now'.", visible_when=("remove_all", False)),
            Option("company", "Company", "text", "",
                   hint="Custom property (shown in Acrobat's Custom tab).",
                   visible_when=("remove_all", False)),
            Option("manager", "Manager", "text", "", visible_when=("remove_all", False)),
            Option("category", "Category", "text", "", visible_when=("remove_all", False)),
            Option("comments", "Comments", "text", "", visible_when=("remove_all", False)),
            ...
            Option("trapped", "Trapped", "choice", "", [
                ...
                ("True", "True"),
                ("False", "False"),
            ], hint="Prepress trapping flag.", visible_when=("remove_all", False)),
            Option("show_title", "Show document Title (not file name) in the window bar",
                   "bool", False, visible_when=("remove_all", False)),
            Option("remove_all", "Remove ALL metadata instead (privacy)", "bool", False,
                   hint="Strips every field above plus XMP — ideal before sharing a file."),
        ],
    ),
    Tool(
